...
def crawl():
    ...
    ssrf_results_path = os.path.join(RESULTS_FOLDER, "ssrf_results.csv")
    ssrf_cve_path = os.path.join(RESULTS_FOLDER, "ml_remediation", "ssrf_cve_dataset.csv")
    
    # Dictionaries to store statistics
    attack_type_counts, severity_counts = {}, {}

    # Fetch Attack Type Distribution
    if os.path.exists(ssrf_results_path):
        df_attack = pd.read_csv(ssrf_results_path)

        # Debug: Print DataFrame columns
        print("\n[DEBUG] Columns in ssrf_results.csv:", df_attack.columns)

        # Extract Attack Type distribution
        if "attack_type" in df_attack.columns:
            attack_type_counts = df_attack["attack_type"].value_counts().to_dict()

        # Debug: Print extracted attack types
        print("\n[DEBUG] Extracted Attack Type Distribution:", attack_type_counts)
    else:
        print("\n[ERROR] ssrf_results.csv not found!")

    # Fetch Severity Distribution
    if os.path.exists(ssrf_cve_path):
        df_severity = pd.read_csv(ssrf_cve_path)

        # Debug: Print DataFrame columns
        print("\n[DEBUG] Columns in ssrf_cve_dataset.csv:", df_severity.columns)

        # Ensure Severity Column Exists & Handle Missing Values
        if "Severity" in df_severity.columns:
            severity_counts = df_severity["Severity"].value_counts().to_dict()

            # Debug: Print severity data extracted
            print("\n[DEBUG] Extracted Severity Distribution (No N/A):", severity_counts)
        else:
            print("\n[WARNING] 'Severity' column is missing in ssrf_cve_dataset.csv!")

    else:
        print("\n[ERROR] ssrf_cve_dataset.csv not found!")
